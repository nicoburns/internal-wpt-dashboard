...
async function score_single_run (chunks_dir) {
    ...
    for (const [idx, area_name] of Object.entries(area_keys)) {
        const percent = ((score[idx].total_score / Math.max(score[idx].total_tests, 1)) * 100).toFixed(1) + '%'
        row.push([area_name, percent, score[idx].total_score.toFixed(3), score[idx].total_tests, score[idx].total_subtests_passed, score[idx].total_subtests])
    }
